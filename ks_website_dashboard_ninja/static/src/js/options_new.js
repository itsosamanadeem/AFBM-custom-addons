...
//import  {DateTimePickerWidget} from "@web/public/datetime_picker_widget";
import { Component, useState, onWillStart, useRef } from "@odoo/owl";

class KsdashboardDialog extends Component {
  static template = "ks_website_dashboard_ninja.s_dashboard_dialog";
  static components = { Dialog };
  setup() {
    this.state = useState({ dashboard: [] });
    onWillStart(async () => await this.willstart());
    this.ks_selection = useRef("ks_selection");
  }

  async willstart() {
    var self = this;
    var result = await jsonrpc("/dashboard", {
  ...
  onConfirm(ev) {
    let ks_value = $(this.ks_selection.el).find(":selected").attr("value");
    let ks_dash_id = parseInt(
      $(this.ks_selection.el).find(":selected").attr("id")
    );
    let ks_data_selection = $("input:radio:checked").attr("value");
    this.props.confirm(ks_value, ks_dash_id, ks_data_selection);
    this.props.close();
  }
}
//KsdashboardDialog.template="ks_website_dashboard_ninja.s_dashboard_dialog";
//KsdashboardDialog.components = { Dialog };

options.registry.dashboard_selector_action = options.Class.extend({
    selector: '.ks_dynamic_dashboard',
  xmlDependencies: [
    "/ks_website_dashboard_ninja/static/src/xml/ks_dashboard_ninja_templates.xml",
  ],
  init() {
    ...
    await new Promise(async(resolve) => {
      await this.dialog.add(
        KsdashboardDialog,
        {
          ...
          confirm: (value, id, selection_id) => {
            ks_self.dashboard_id = id;
            ks_self.data_selection = selection_id;
            ks_self.startdashboard(id, selection_id, ks_self);
          },
        },
        {
          onClose: resolve,
  ...
  startdashboard(dashboard_id, data_selection, ks_self_received) {
    var ks_self = this;
        ...
        if (data !== "missingerror") {
          if (!data.login) {
            ...
            if (data.type === "user_data") {
              $(renderToElement("ks_website_dashboard_ninja.KsWebsiteNoItemNoUserView")).appendTo(
                ks_self.$el
              );
  ...
  _renderDateFilterDatePicker: function () {
    ...
    ks_self.$el.find("#datetimepicker1").each((index, item) => {
  ...
  destroy() {
    this._super(...arguments);
    this.$target.off(".snippet_dashboard_home_page");
  },
});

export default {
  dashboard_selector_action: options.registry.dashboard_selector_action,
};
